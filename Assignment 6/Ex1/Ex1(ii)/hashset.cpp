...
#include <cstdlib>
#include "hashset.h"
#define outspace (maxsize / 2)
#define inspace  (maxsize / 2)
using std::cout;
using std::cin;
using std::hash;
...
template<class T> void hashset<T>::add(T item)
{
    ...
    int index;
    index = hashfunction(item) % inspace; // First determine the position index in the hash table, where the new value is stored, if free.
    ...
    *pt = item;
    ...
    ++ numitems;
    if(location >= inspace)  exitems ++;
    int load = 100 * numitems / maxsize;
    int outload = 100 * exitems / outspace;
    if (load >= 75 || outload >= 75)             // max load factor is exceeded; double the size
    {
...
template<class T> void hashset<T>::remove(T item)
{
    ...
    int index;
    index = hashfunction(item) % inspace;
...
template<class T> bool hashset<T>::member(T item)
{
    ...
    while (reprarray[index] != 0) // again we search for item starting from the index position until we find a null pointer
    {
        ...
        index = index + 1;  // it will not overflow
    }
...
template<class T> void hashset<T>::rehash(int newsize)
{
    ...
    for (int i = 0; i < maxsize; i++)  // we need to copy all existing entries to the new hash table
    {
        if (reprarray[i] != 0 && reprarray[i] != pt_nil)
        {
            T item = *reprarray[i];
            hash<T> hashfunction;
            int index = hashfunction(item) % (newsize / 2);
