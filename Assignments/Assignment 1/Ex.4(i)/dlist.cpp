...
template<class T> void DList<T>::selectionSort()
{
    node<T> *pt1 = dummy;
    for(int i = 1; i < numitems; i++){     //iterate the position to be filled into
        pt1 = pt1->getnext();
        node<T> *pt2 = pt1, *smp = pt1;
        for(int j = i+1; j <= numitems; j++){   //find the smallest element
            pt2 = pt2->getnext();
            if(pt2->dataitem < smp->dataitem)
                smp = pt2;
        }
        T val = pt1->dataitem;  //swap with the smallest element selected
        pt1->dataitem = smp->dataitem;
        smp->dataitem = val;
    }
}
