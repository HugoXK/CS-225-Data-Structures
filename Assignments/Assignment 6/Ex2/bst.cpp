 ...
 template<class T> node<T> *BST<T>::_delete_with_reinsert(node<T> *pt, T val)
{
    ...
    {   
        pt_new = (*pt).getleft();
        _reinsert((*pt).getleft(),pt_new);
        _reinsert((*pt).getright(),pt_new);
        return pt_new;
    }
    /* The recursive descent follows the left subtree or the right subtree. The call of _delete returns a pointer to the root of the modified tree. This must be returned and stored in the position (left or right) that issued the call.*/
    if (val < (*pt).getdata())
    {
        pt_new = _delete_with_reinsert((*pt).getleft(), val);
        (*pt).setleft(pt_new);
    }
    else
    {
        pt_new = _delete_with_reinsert((*pt).getright(), val);
        (*pt).setright(pt_new);
    }
    return pt;
}
...
template<class T> void BST<T>::_reinsert(node<T> *pt1,node<T> *pt2) 
{
    if (pt1 == 0 )
    {
        return;
    ...
    {
        _insert(pt2,(*pt1).getdata());
        _reinsert((*pt1).getleft(),pt2);
        _reinsert((*pt1).getright(),pt2);
    }  
}
